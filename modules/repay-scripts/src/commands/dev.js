...
const transpileWithBabel = async (input, options) => {
...
const runWithRollup = async (input, options) => {
  ...
  if (options.treeShaking) {
    await transpileWithBabel(input, options)
  }
}

const tapCompilerHooks = (compiler, port) => {
  let isFirstCompile = true
...
const runWithWebpack = async (input, options) => {
  ...
  tapCompilerHooks(compiler, PORT)
...
async function dev(options) {
  ...
  if (isLibrary) {
    await runWithRollup(input, options)
  } else {
    await runWithWebpack(input, options)
  }
}
